...
Vst.prototype.getVideoCurrentTime = function(video) {
	var ss = 0;
	var tmp = video.src.split('?s=');
...
Vst.prototype.updateVideoSeekbar = function(video) {
	...
	if (!obj.cache.events.videoTimeupdate) {
		...
		video.addEventListener('timeupdate', function() {
			var df = obj.formatTime(obj.cache.videoDuration, true);
			var c = obj.getVideoCurrentTime(video) + video.currentTime;
			var p = obj.calcPart(c, obj.cache.videoDuration);
	...
	$('#seekbarOuter').unbind('click').click(function(e) {
		...
		var nx = obj.cache.videoDuration * (p / 100);
	...
	$('#seekbarOuter').bind('mousemove', function(e) {
		...
		var c = e.originalEvent.layerX;
		var p = obj.calcPart(c, d);
		var dx = obj.cache.videoDuration;
...
Vst.prototype.videoVolume = function(video) {
	...
	$('#volumeOuter').unbind('mouseup').mouseup(function(ev) {
		clickEv(ev, this);
		if (down) {
			down = false;
		}
		...
		down = true;
	}).unbind('mouseout').mouseout(function(ev) {
		if (down) {
			clickEv(ev, this);
			down = false;
		}
...
Vst.prototype.videoEvents = function(modal) {
	...
	video.addEventListener('loadstart', function() {
		if (!video.src.match(/\?p=tv/) && video.src !== '') {
	...
	video.addEventListener('canplaythrough', function() {
		...
		setTimeout(function() {
			obj.cache.videoDuration = video.duration;
			video.play();
			$('#player')

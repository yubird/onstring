...
Vst.prototype.loading = function(state, message) {
	...
	if (typeof(message) === 'string' && message.length > 0) {
		var loader = [
			'<div id="loader" class="ui text loader" style="min-height:',
			'40vh;">', message, '</div>'
		];
	} else {
		var loader = [
			'<div id="loader" class="ui loader" style="min-height:',
			'40vh;"></div>'
		];
	}
	...
	if (rec.isFile) {
		if (rec.isPlayable) {
			...
			if (!rec.fullPath.match('.ts')) {
				str.push(
					'<a href="', link, '" class="item play" data-dl="',
		...
		str.push(
			...
			'<a href="',
			dlLink,
			'" target="_blank"><div class="ui content">',
...
Vst.prototype.setPlayer = function(modal, url, exUrl) {
	...
	var posterUrl = obj.imageUrl(url);
	var timer = false;
	var sdvUrl = 'http://sdv.string.tokyo' + url;
	...
	if (isRealAnime) {
		if (isMobile) {
			...
			obj.sizeSelector(modal, sdvUrl, false, posterUrl);
			$(modal).find('video')
				.attr('poster', '/img/video-loading.gif')
		}
	} else {
		$.ajax({
			url: sdvCUrl,
			...
			success: function(res) {
				if (res === '1') {
					...
					if (isMobile) {
						...
						obj.sizeSelector(modal, url, sdvUrl, posterUrl);
					}
				} else {
					if (isMobile) {
						location.href = url;
					} else {
						obj.sizeSelector(modal, url, false, posterUrl);
					}
				}
			}
		});
	...
	if (!isMobile) {
		...
		$(modal).find('video').focus(function() {
...
Vst.prototype.videoController = function() {
	...
	var showController = function() {
		if (!obj.cache.events.videoControllTimer) {
			clearTimeout(obj.cache.events.videoControllTimer);
			obj.cache.videoControllTimer = false;
		}
		if ($(controller).is(':hidden')) {
			$(controller).slideDown(300, function() {
				obj.cache.events.videoControllTimer = setTimeout(function() {
	...
	$('#player, #controller, #seekbar').mousemove(function() {
		showController();
	});
};
...
Vst.prototype.updateVideoSeekbar = function(video) {
	...
	if (!obj.cache.events.videoTimeupdate) {
		...
		$(document)
			.bind(
				'webkitfullscreenchange mozfullscreenchange fulscreenchange',
				function() {
	...
	var tTimer = false;
	$('#seekbar').unbind('mousemove').mousemove(function(e) {
	...
	$('#controller > i.expand').unbind('click').click(function() {
		if (!obj.isFullscreen()) {
			if (player.webkitRequestFullscreen) {
				...
				player.mozRequestFullScreen(); //FF10+
			} else if (player.msRequestFullscreen) {
				player.msRequestFullscreen(); //IE11+
			} else if (player.requestFullscreen) {
				player.requestFullscreen(); // HTML5 Fullscreen API仕様
			}
		} else {
 ...
 */
Vst.prototype.sizeSelector = function(modal, hdUrl, sdUrl, posterUrl) {
	var obj = this;
...
Vst.prototype.videoEvents = function(modal) {
	...
	video.addEventListener('pause', function() {
		...
		$(modal).find('#videoInfo').fadeIn(200);
 ...
 * @return String
 */
Vst.prototype.pageName = function() {
	var obj = this;
	var name = decodeURIComponent(this.cache.params.sub)
		.replace('/', '&nbsp;>&nbsp;');
	var base = this.cache.params.p === 'anime'? 'アニメ': 'TV番組';
...
Vst.prototype.writeRules = function() {
	...
	obj.getRules(function(res) {
		var str = [
			...
			'<thead>',
			'<tr>',
			'<th colspan="2">', obj.pageName(), '</th></tr><tr>',
			'<th>キーワード</th>',
...
Vst.prototype.getShuffledMusic = function(items) {
	...
	if (
		...
		return obj.getShuffleMusic(items);
	}
 ...
 * @return String
 */
Vst.prototype.formatTime = function(time) {
	var min = ('00' + ~~(time / 60)).slice(-2);
...
Vst.prototype.playRealTimeTv = function() {
	...
	obj.getChannels(function(res) {
		...
		$(selector).find('div.channel').click(function() {
			var uid = (new Date()).getTime();
			var videoUrl = 'http://sdv.string.tokyo/realtv/'
				+ $(this).attr('data-ch')
				+ '/watch.webm?s=' + uid;
			$(selector).hide();
			$(modal).find('video')

	...
	obj.getList(function(res) {
		...
		$(obj.cache.elements.content).find('a').click(function() {
			if ($(this).hasClass('play')) {
				...
				obj.savePlayedVideo(obj.cache.params.sub, $(this).find('span').text());
...
Vst.prototype.createRow = function(rec) {
	...
	if (rec.isFile) {
		if (rec.isPlayable) {
			isPlayed = obj.getPlayedVideo(obj.cache.params.sub, rec.name);
			str.push(
				'<a href="', link,
				'" class="item ',
				(isPlayed? 'played ': ''),
	...
	setTimeout(function() {
		for (var i = 0; i < list.length; ++i) {
			...
			if ($(cur).attr('data-play') === playUrl) {
				if (i < list.length - 1) {
					obj.savePlayedVideo(
						obj.cache.params.sub, $(list).eq(i + 1).find('span').text()
					);
...
Vst.prototype.savePlayedVideo = function(sub, name) {
	...
	if (typeof(playedVideo[key]) === 'undefined') {
		playedVideo[key] = 1;
